...
class IrrigationControllers(Document):
    ...
    def check_and_save_controller(deviceName = get_device_name(), deviceType = "", ipAddress = get_ip_address(), status = "Active"):
        # Check if a controller with the same ID already exists
        existing_controller = IrrigationControllers.objects(deviceName=deviceName).first()
